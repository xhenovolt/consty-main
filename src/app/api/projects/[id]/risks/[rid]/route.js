...
export async function PATCH(request, { params }) {
  ...
  try {
    ...
    const ALLOWED = ['description','probability','impact','mitigation_plan','owner_id','status'];
    const sets = [], values = [];
    ...
    if (!rows[0]) return NextResponse.json({ success: false, error: 'Risk not found' }, { status: 404 });
    return NextResponse.json({ success: true, data: rows[0] });
  } catch (error) {
    console.error('[Risks] PATCH error:', error);
...
export async function DELETE(request, { params }) {
  ...
  const gate = await assertProjectAccess(perm.auth, id, { write: true });
  if (!gate.ok) return NextResponse.json({ success: false, error: gate.error }, { status: gate.status });
  await query(`DELETE FROM risks WHERE id = $1 AND project_id = $2`, [rid, id]);
